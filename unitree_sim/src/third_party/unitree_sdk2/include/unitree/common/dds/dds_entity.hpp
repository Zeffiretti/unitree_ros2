...
class DdsParticipant : public DdsLogger {
 public:
  ...
  explicit DdsParticipant(uint32_t domainId, const DdsParticipantQos& qos,
...
class DdsPublisher : public DdsLogger {
 public:
  ...
  explicit DdsPublisher(const DdsParticipantPtr& participant,
                        const DdsPublisherQos& qos);
  ~DdsPublisher();
...
class DdsSubscriber : public DdsLogger {
 public:
  ...
  explicit DdsSubscriber(const DdsParticipantPtr& participant,
                         const DdsSubscriberQos& qos);
  ~DdsSubscriber();
...
class DdsTopic : public DdsLogger {
 public:
  ...
  explicit DdsTopic(const DdsParticipantPtr& participant,
                    const std::string& name, const DdsTopicQos& qos)
  ...
  ~DdsTopic() { mNative = __UT_DDS_NULL__; }

  const NATIVE_TYPE& GetNative() const { return mNative; }
...
class DdsWriter : public DdsLogger {
 public:
  ...
  explicit DdsWriter(const DdsPublisherPtr publisher,
                     const DdsTopicPtr<MSG>& topic, const DdsWriterQos& qos)
      : mNative(__UT_DDS_NULL__) {
    UT_DDS_EXCEPTION_TRY
    ...
    mNative =
        NATIVE_TYPE(publisher->GetNative(), topic->GetNative(), writerQos);

    UT_DDS_EXCEPTION_CATCH(mLogger, true)
  }

  ~DdsWriter() { mNative = __UT_DDS_NULL__; }
...
class DdsReaderListener : public ::dds::sub::NoOpDataReaderListener<MSG>,
 ...
 public:
  ...
  void SetQueue(int32_t len) {
    ...
    mDataQueueThreadPtr =
        CreateThreadEx("rlsnr", UT_CPU_ID_NONE, queueThreadFunc);
  }

  int64_t GetLastDataAvailableTime() const { return mLastDataAvailableTime; }

  NATIVE_TYPE* GetNative() const { return (NATIVE_TYPE*)this; }

  const ::dds::core::status::StatusMask& GetStatusMask() const { return mMask; }
      ...
      if (iter->info().valid()) {
        ...
        if (mHasQueue) {
          if (!mDataQueuePtr->Put(MSG_PTR(new MSG(m)), true)) {
            LOG_WARNING(mLogger, "earliest mesage was evicted. type:",
                        DdsGetTypeName(MSG));
          }
        } else {
...
class DdsReader : public DdsLogger {
 public:
  ...
  explicit DdsReader(const DdsSubscriberPtr& subscriber,
                     const DdsTopicPtr<MSG>& topic, const DdsReaderQos& qos)
      : mNative(__UT_DDS_NULL__) {
    UT_DDS_EXCEPTION_TRY
    ...
    mNative =
        NATIVE_TYPE(subscriber->GetNative(), topic->GetNative(), readerQos);

    UT_DDS_EXCEPTION_CATCH(mLogger, true)
  }

  ~DdsReader() { mNative = __UT_DDS_NULL__; }
