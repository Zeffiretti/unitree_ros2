...
class DdsTopicChannel : public DdsTopicChannelAbstract {
 public:
  ...
  void SetTopic(const DdsParticipantPtr& participant, const std::string& name,
                const DdsTopicQos& qos) {
  ...
  void SetReader(const DdsSubscriberPtr& subscriber, const DdsReaderQos& qos,
                 const DdsReaderCallback& cb, int32_t queuelen) {
    mReader = DdsReaderPtr<MSG>(new DdsReader<MSG>(subscriber, mTopic, qos));
    mReader->SetListener(cb, queuelen);
  }

  DdsWriterPtr<MSG> GetWriter() const { return mWriter; }

  DdsReaderPtr<MSG> GetReader() const { return mReader; }
