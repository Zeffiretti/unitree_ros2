...
class low_level_cmd_listener : public rclcpp::Node {
 public:
  low_level_cmd_listener() : Node("low_level_cmd_sender") {
    ...
    timer_ = this->create_wall_timer(
        ...
        std::bind(&low_level_cmd_listener::timer_callback, this));
 ...
 private:
  void timer_callback() {
    ...
    std::cout << "t: " << t << " RL_0 q: " << cmd_msg.motor_cmd[RL_0].q
              << " RL_2 tau: " << cmd_msg.motor_cmd[RL_2].tau << std::endl;
...
int main(int argc, char **argv) {
  ...
  auto node =
      std::make_shared<low_level_cmd_listener>();  // Create a ROS2 node and
                                                   // make share with
