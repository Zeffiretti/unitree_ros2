...
void UiEvent(mjuiState* state) {
  ...
  if ((state->dragrect == sim->ui0.rectid) ||
    ...
    } else if (it && it->sectionid == SECT_OPTION) {  // option section
    ...
    } else if (it && it->sectionid == SECT_SIMULATION) {  // simulation section
      switch (it->itemid) {
        case 1:  // Reset
    ...
    } else if (it && it->sectionid == SECT_RENDERING) {  // rendering section
    ...
    } else if (it && it->sectionid == SECT_GROUP) {  // group section
...
void Simulate::Render() {
  ...
  if (this->screenshotrequest.exchange(false)) {
    ...
    // location. Thus, we defer to a default "screenshot.png" for now. const
    // std::string path = GetSavePath("screenshot.png"); if (!path.empty()) {
