...
class RobotCfgs:
    class Go2:
        NUM_DOF = 12
        NUM_ACTIONS = 12
        # from isaacgym joint order to real robot joint order
        dof_map = [3, 4, 5, 0, 1, 2, 9, 10, 11, 6, 7, 8]
        dof_names = [  # NOTE: order matters. This list is the order in isaacgym.
            "FL_hip_joint",
            "FL_thigh_joint",
            "FL_calf_joint",
            "FR_hip_joint",
        ...
        torque_limits = torch.tensor(
            [25, 40, 40, 25, 40, 40, 25, 40, 40, 25, 40, 40],  # from urdf and in isaacgym order
            device="cpu",
            dtype=torch.float32,
        )
...
class UnitreeRos2Real(Node):
    ...
    def init_buffer(self):
        ...
        self.kp = 40.0
        self.kd = 1.0
        self.p_gains = [self.kp] * self.NUM_DOF
        self.d_gains = [self.kd] * self.NUM_DOF
        ...
        if self.computer_clip_torque:
            assert hasattr(self, "torque_limits") and (
                len(self.torque_limits) == self.NUM_DOF
            ), f"torque_limits must be set with the length of {self.NUM_DOF} if computer_clip_torque is True"
    ...
    def start_ros_handlers(self):
        ...
        #         Image, self.forward_depth_topic, self._forward_depth_callback, 1
        #     )
        ...
        self.get_logger().info(
            "ROS handlers started, waiting to recieve critical low state and wireless controller messages."
        )
        if not self.dryrun:
            self.get_logger().warn(
                f"You are running the code in no-dryrun mode and publishing to '{self.low_cmd_topic}', Please keep safe."
            )
        else:
            self.get_logger().warn(
                f"You are publishing low cmd to '{self.low_cmd_topic}' because of dryrun mode, Please check and be safe."
            )
        while rclpy.ok():
    ...
    def _low_state_callback(self, msg):
        ...
        for sim_idx in range(self.NUM_DOF):
            real_idx = self.dof_map[sim_idx]
            if (
                self.dof_pos_[0, sim_idx] > self.joint_pos_protect_high[sim_idx]
                or self.dof_pos_[0, sim_idx] < self.joint_pos_protect_low[sim_idx]
            ):
    ...
    def _forward_depth_embedding_callback(self, msg):
        self.forward_depth_embedding_buffer = torch.tensor(
            msg.data, device=self.model_device, dtype=torch.float32
        ).view(1, -1)
    ...
    def _get_projected_gravity_obs(self):
        ...
        return quat_rotate_inverse(quat_xyzw, self.gravity_vec)
    ...
    def _get_orientation_cmds_obs(self):
        return quat_rotate_inverse(
            quat_from_euler_xyz(
                self.roll_pitch_yaw_cmd[:, 0],
                self.roll_pitch_yaw_cmd[:, 1],
                self.roll_pitch_yaw_cmd[:, 2],
            ),
            self.gravity_vec,
        )
    ...
    def get_obs_segment_from_components(self, components):
        ...
        if "lin_vel" in components:
            print(
                "Warning: lin_vel is not typically available or accurate enough on the real robot. Will return zeros."
            )
            segments["lin_vel"] = (3,)
        ...
        #         segments["forward_depth"] = (
        #             1,
        #             *self.cfg["sensor"]["forward_camera"]["output_resolution"],
        #         )
        #     else:
        #         segments["forward_depth"] = (
        #             1,
    ...
    def send_action(self, actions):
        ...
        if self.computer_clip_torque:
            ...
            self.get_logger().warn(
                "Computer Clip Torque is False, the robot may be damaged.",
    ...
    def _publish_legs_cmd(self, robot_coordinates_action: torch.Tensor):
        ...
        for sim_idx in range(self.NUM_DOF):
            ...
            self.low_cmd_buffer.motor_cmd[real_idx].q = (
                robot_coordinates_action[sim_idx].item() * self.dof_signs[sim_idx]
            )
